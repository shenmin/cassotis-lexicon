...
| 文件 | 变体 | 词条数 |
|------|------|--------|
| `data/generated/dict_clean_sc.txt` | 简体主词库 | 187,702 |
| `data/generated/dict_clean_tc.txt` | 繁体主词库 | 190,789 |
...
|---|---:|---:|
| `v1.2.0` | 152,729 / 145,284 | 698,695 / 698,778 |
| `v1.1.0` | 152,729 / 145,284 | 558,645 / 559,128 |
| `v1.0.0` | — | — |

以上记录不是可直接输入的词条，因此不计入前述词库快照。词级二元、三元转移关系由 Lexicon 语料构建流程生成；带平滑回退的字符级 n-gram 模型由 IME 训练工具生成，再由本仓库统一版本化和发布。Cassotis IME 会将两类产物导入本地 SQLite 词库，用于长句 lattice/N-best 路径打分和完整候选的保守重排。

模型只保存短距离词语转移和字符 n-gram，不保存完整训练句子。“长句基准测试-16300”语料不参与训练。运行时评分完全在本地完成，不需要网络连接、GPU 或神经网络运行环境。评测方法见 [IME 基准测试说明](https://github.com/shenmin/cassotis-ime/blob/v1.2.0/BENCHMARK.CN.md)。
