...
## 当前词库快照（2026-09-13 构建）

| 文件 | 变体 | 词条数 |
|------|------|--------|
| `data/generated/dict_clean_sc.txt` | 简体主词库 | 189,435 |
| `data/generated/dict_clean_tc.txt` | 繁体主词库 | 192,391 |
| `data/generated/dict_unihan_sc.txt` | 简体单字（Unihan） | 23,913 |
| `data/generated/dict_unihan_tc.txt` | 繁体单字（Unihan） | 24,172 |
...
版本区间表示所列类别的发布产物保持一致。统计数量不包含文件表头。
...
构建流程还会从强证据 exact 词路径离线生成独立的转移补全索引，每个输入前缀最多保留六个经过复核的可靠方案。用户补全仍优先；基础词库与转移补全方案结合流行度、上下文和语料证据比较。该索引不计入可直接输入的词条数据。

### 一键补全模型数据

| 版本 | 补全流行度先验（简体 / 繁体） | 前缀查询记录（简体 / 繁体） | 转移补全索引（简体 / 繁体） |
|---|---:|---:|---:|
...
v1.16.0 引入基于语料的补全流行度先验和预先构建的 Top-K 前缀查询索引，让 IME 无需运行时枚举组合，就能区分热门补全与冷门专业词。流行度先验是独立构建的快照，其数量不必与当前主词库一致。

用于补全排序和局部续写的其他静态数据：

| 版本 | 补全竞争先验（简体 / 繁体） | 成对换位审计记录（简体 / 繁体） | 长句局部续写索引（简体 / 繁体） |
|---|---:|---:|---:|
| `v1.18.0` - `v1.23.0` | 42,453 / 42,448 | 4,379 / 4,379 | 97,589 / 92,457 |
| `v1.17.0` | 42,453 / 42,448 | 4,379 / 4,379 | 35,423 / 32,481 |

v1.17.0 起，竞争先验与成对审计用于补全选择和弃权判断，独立的续写索引则为稳定词锚点提供短距离后续。这些是静态查询记录，不是新增词条或神经网络参数；由 IME 项目训练和部署的模型不计入这些表格。
