...
## 当前词库快照（2026-08-17 构建）

| 文件 | 变体 | 词条数 |
|------|------|--------|
| `data/generated/dict_clean_sc.txt` | 简体主词库 | 188,961 |
| `data/generated/dict_clean_tc.txt` | 繁体主词库 | 191,854 |
...
构建流程还会从强证据 exact 词路径离线生成独立的转移补全索引。每个输入前缀至多保留一个经过复核的局部补全，优先级低于用户词和基础词库 exact 补全，也不计入可直接输入的词条数据。
