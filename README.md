...
These records are not directly inputable dictionary entries and are therefore excluded from the dictionary snapshot counts above. The word bigram/trigram transitions are generated by the Lexicon corpus-build pipeline. The smoothed character trigram models are generated by the IME training tools, then versioned and distributed from this repository. Cassotis IME imports both artifacts into its local SQLite dictionaries for long-sentence lattice/N-best path scoring and conservative complete-candidate reranking.

The model stores only short word transitions and character n-grams, not complete training sentences. The independent Benchmark-16300 corpus is not used for training. Runtime scoring is local and requires no network connection, GPU, or neural-network runtime. See the [IME benchmark documentation](https://github.com/shenmin/cassotis-ime/blob/v1.1.0/BENCHMARK.md) for the evaluation protocol.
