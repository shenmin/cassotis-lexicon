...
## Current dictionary snapshot (2026-09-10 build)

| File | Variant | Entries |
|------|---------|---------|
| `data/generated/dict_clean_sc.txt` | Simplified Chinese | 189,378 |
| `data/generated/dict_clean_tc.txt` | Traditional Chinese | 192,334 |
| `data/generated/dict_unihan_sc.txt` | Simplified single-char (Unihan) | 23,912 |
| `data/generated/dict_unihan_tc.txt` | Traditional single-char (Unihan) | 24,171 |

## Local ranking model data and AI reranking support

Cassotis Lexicon publishes local ranking artifacts alongside its lexical dictionaries. Query-path priors were already available in v1.0.0; v1.1.0 added corpus-trained statistical language-model data. These artifacts improve long-sentence and short-word contextual ranking rather than substantially increasing the number of directly inputable words.
...
Version ranges indicate identical published artifacts in the listed categories. Counts exclude file headers.

These records are not directly inputable dictionary entries and are therefore excluded from the dictionary snapshot counts above. The query-path priors and word bigram/trigram transitions are generated by the Lexicon build pipeline. The forward and reverse smoothed character n-gram models are generated by the IME training tools, then versioned and distributed from this repository. Cassotis IME imports these artifacts into its local SQLite dictionaries for long-sentence lattice/N-best path scoring and, starting with v1.4.0, as contextual evidence when ranking ambiguous short exact candidates.

The build also derives a separate offline transition-completion index from strongly supported exact-word paths. It retains up to six vetted alternatives for each typed prefix. User completions remain the first priority; base-dictionary and transition suggestions are compared using popularity, context, and corpus evidence. The index is not counted as directly inputable dictionary data.

### One-key completion model data
...
The v1.13.0 artifacts add reverse character n-gram models for bidirectional path scoring. Query-path priors increase slightly, while word-transition and forward character-model counts remain unchanged.
...
## Directory layout
- `data/generated/`: generated lexicon files, versioned language-model artifacts, and offline transition-completion indexes.
