...
## Current dictionary snapshot (2026-09-04 build)

| File | Variant | Entries |
|------|---------|---------|
| `data/generated/dict_clean_sc.txt` | Simplified Chinese | 189,322 |
| `data/generated/dict_clean_tc.txt` | Traditional Chinese | 192,214 |
...
The build also derives a separate offline transition-completion index from strongly supported exact-word paths. It retains up to six vetted alternatives for each typed prefix so runtime context can select one stable suggestion; the index remains lower priority than user and base exact completions and is not counted as directly inputable dictionary data.

### One-key completion model data

| Version | Completion popularity priors (SC / TC) | Prefix lookup records (SC / TC) |
|---|---:|---:|
| `v1.16.0` | 188,983 / 191,875 | 255,129 / 270,714 |

v1.16.0 adds a corpus-derived popularity prior for every base completion and a precomputed Top-K prefix lookup index. These records let IME distinguish popular exact completions from cold vertical-domain terms without enumerating combinations at runtime; they are model data rather than additional inputable words.
