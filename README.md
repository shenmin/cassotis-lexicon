...
## Current dictionary snapshot (2026-07-21 build)

| File | Variant | Entries |
|------|---------|---------|
| `data/generated/dict_clean_sc.txt` | Simplified Chinese | 187,762 |
| `data/generated/dict_clean_tc.txt` | Traditional Chinese | 190,851 |
...
The statistical model stores only short word transitions and character n-grams, not complete training sentences. The independent Benchmark-16300 corpus is not used for training. When a user enters a long sentence, IME evaluates the neural reranker directly from its deterministic native parameters while producing candidates; it starts no PyTorch/ONNX runtime or external model service and requires no network connection or GPU. See the [IME benchmark documentation](https://github.com/shenmin/cassotis-ime/blob/master/BENCHMARK.md) for the evaluation protocol.
...
## Directory layout
- `data/generated/`: generated lexicon files and versioned language-model artifacts.
