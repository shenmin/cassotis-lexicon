...
|------|---------|---------|
| `data/generated/dict_clean_sc.txt` | Simplified Chinese | 187,702 |
| `data/generated/dict_clean_tc.txt` | Traditional Chinese | 190,789 |
...
## Corpus-trained AI model data

Cassotis Lexicon v1.1.0 begins publishing local statistical language-model data alongside the lexical dictionaries. This release focuses on adding contextual ranking data rather than substantially increasing the number of directly inputable words.

| Version | Word-path transitions (SC / TC) | Character n-gram parameters (SC / TC) |
|---|---:|---:|
| `v1.2.0` | 152,729 / 145,284 | 698,695 / 698,778 |
| `v1.1.0` | 152,729 / 145,284 | 558,645 / 559,128 |
| `v1.0.0` | — | — |

These records are not directly inputable dictionary entries and are therefore excluded from the dictionary snapshot counts above. The word bigram/trigram transitions are generated by the Lexicon corpus-build pipeline. The smoothed character n-gram models are generated by the IME training tools, then versioned and distributed from this repository. Cassotis IME imports both artifacts into its local SQLite dictionaries for long-sentence lattice/N-best path scoring and conservative complete-candidate reranking.

The model stores only short word transitions and character n-grams, not complete training sentences. The independent Benchmark-16300 corpus is not used for training. Runtime scoring is local and requires no network connection, GPU, or neural-network runtime. See the [IME benchmark documentation](https://github.com/shenmin/cassotis-ime/blob/v1.2.0/BENCHMARK.md) for the evaluation protocol.
